...
// token is invalid if the user logged out of Facebook.
	...
	foreach($posts['data'] as $post){
		...
		echo "<span class='post'>{ \"text\":\"".substr($preview,0,120)."....\", \"likes\": ".$numLikes.", \"comments\": ".$numComments.", \"date\": \"". date('g:i a l, F j',strtotime($post['created_time']))."\" }</span>";
  ...
  */  
  ...
  void init(){
    ...
    for(int j=0;j< count;j++){
      ...
      e[j][0]=theX; // X 
      e[j][1]=theY; // Y
      e[j][2]=(arr_posts[j].likes/int_topLikes)*100; //first radius - based on likes        
      e[j][3]=0; // X Speed
      e[j][4]=0; // Y Speed    
      e[j][5]=(arr_posts[j].comments/int_topLikes)*100 //inner size - based on comments
    }
  }
